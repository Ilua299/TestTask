		...
		public function gelAllCollegies()
		{
			...
			for ($i = 1; $i <= $pageNumbers;$i++)
			{
				...
				foreach ($collegies_names as $key => $value) {
					mysqli_query($mysqli,"INSERT IGNORE INTO allcollegies(college_id,image_src,college_name,city,state) VALUES ('".$mysqli->real_escape_string($this->CollegiesID[$college_name[$key]])."','".$mysqli->real_escape_string($images_src[$key])."','".$mysqli->real_escape_string($college_name[$key])."','".$mysqli->real_escape_string($city[$key])."','".$mysqli->real_escape_string($state[$key])."')");
					$this->getCollege($this->CollegiesID[$college_name[$key]],$value,$mysqli);
				}
			}
			$this->deleteOldCollegies($mysqli);
			$mysqli->close();
		}

		private function getCollege($college_id,$collegies_name, $mysqli)
		{
			...
			mysqli_query($mysqli,"INSERT IGNORE INTO college(college_id,college_name,address,phone,site) VALUES ('".$mysqli->real_escape_string($college_id)."','".$mysqli->real_escape_string($college_name)."','".$mysqli->real_escape_string($address)."','".$mysqli->real_escape_string($phone)."','".$mysqli->real_escape_string($site)."')");
		}

		private function deleteOldCollegies($mysqli)
		{
			...
			foreach ($oldCollegies as $row) {
				$sql = "DELETE FROM allcollegies WHERE college_id = '".$mysqli->real_escape_string($row)."'";
				mysqli_query($mysqli,$sql);
				$sql = "DELETE FROM college WHERE college_id = '".$mysqli->real_escape_string($row)."'";
				mysqli_query($mysqli,$sql);
			}
		}
	}
